...
// ⚡ أهم سطر لضمان عمل الروابط على Render: تعريف مجلد الملفات
...
// دالة تحويل روابط يوتيوب لتعمل داخل المشغل
...
// جلب الدردشة (رسائل العميل + ردود الإدارة)
...
app.post('/api/admin/ad/add', async (req, res) => {
    ...
    const newAd = new Ad({ videoUrl: makeEmbedUrl(req.body.videoUrl), active: true });
    await newAd.save();
    res.json({ success: true });
});
...
app.post('/api/admin/user/update-balance', async (req, res) => {
    ...
    await new Message({ receiver: req.body.phone, title: "تحديث المحفظة 💰", body: `تم تحديث رصيدك. الرصيد الجديد: ${req.body.newBalance} YER` }).save();
    res.json({ success: true });
});
...
app.listen(PORT, () => console.log(`🚀 منظومة أبو حسين تعمل على المنفذ ${PORT}`));
